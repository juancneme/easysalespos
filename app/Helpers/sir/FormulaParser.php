 ...
 */
namespace App\Helpers\Sir\FormulaParser;
...
class FormulaParser implements IFormulaParser {
    ...
    private function calculate1(array $array) {
        for ($i = count($array) - 1; $i >= 0; $i--) {
            if (isset($array[$i]) && isset($array[$i - 1]) && isset($array[$i + 1]) && $array[$i] === '^') {
                $otp = 1;
    ...
    private function calculate2(array $array) {
        for ($i = 0; $i < count($array); $i++) {
            if (isset($array[$i]) && isset($array[$i - 1]) && isset($array[$i + 1]) && ($array[$i] === '*' || $array[$i] === '/')) {
                $this->checkInf($array[$i - 1], $array[$i + 1]);
                if (!is_numeric($array[$i - 1]) || !is_numeric($array[$i + 1])) {
                    $this->correct = 0;
                    break;
    ...
    private function calculateFunction($function, &$str, &$strlen, $i) {
        ...
        while (true) {
            if (isset($str[$j])) {
                if ((strstr('+-', $str[$j]) && $arg === null) || preg_match('/([\d\.eE\+\-])|([\d\.INF])/', $str[$j])) {
                    $arg .= $str[$j];
    ...
    private function combine($array) {
        ...
        foreach ($array as $item) {
            ...
            if ((strstr('+-', $item)) && (strstr('+-', $cur)) && (strstr('+-', $array_ip1))) {
                ...
                if ((strstr('+-', $item)) && ($key + 1 != $i) && (strstr('*/^', $array[$key])) && (!is_numeric($array[$key + 1])) && (isset($array[$key + 1]))) {
                    if ($item === '-')
                        $new_array[key($new_array)] = ($cur == '+') ? '-' : '+';
                } else {
                    $new_array[] = $item;
        ...
        foreach ($array as $item) {
            ...
            if ($item === '+' || $item === '-') {
                if (($j == 0 && is_numeric($array_ip1)) || ($j > 0 && is_numeric($array_ip1) && stristr('+-*/^e', $array_im1))) {
                    if ($item === '+') {
                        $new_array[] = $array_ip1;
                    } else {
            ...
            } elseif (($j == 1 && is_numeric($item) && strstr('+-', $array_im1)) || ($j > 1 && is_numeric($item) && strstr('+-', $array_im1) && (stristr('+-*/^e', $array_im2)))) {
    ...
    private function parse($str) {
        ...
        for ($i = 0; $i < $strlen; $i++) {
            ...
            if ($str_i == ' ') {
            ...
            } elseif (($str_i . $str_ip1 == 'pi') && (!$str_ip2 || strstr('+-*/^ ', $str_ip2))) {
                $count++;
                $main_array[$count] = M_PI;
                // Constant e
            } elseif (($str_i == 'e') && ($str_ip1 != 'x') && (!is_numeric($str_im1)) && (!$str_ip1 || strstr('+-*/^ ', $str_ip1))) {
                $count++;
                $main_array[$count] = M_E;
                // Number in E notation
            } elseif ((strtolower($str_i) == 'e') && ($str_ip1 != 'x') && (is_numeric($str_im1))) {
                $main_array[$count] = $main_array[$count] . 'E';
            } elseif ((strstr('+-', $str_i)) && (strtolower($str_im1) == 'e') && (is_numeric($str_ip1)) && (is_numeric($str_im2))) {
                $main_array[$count] = $main_array[$count] . $str_i;
                // Decimal point
            } elseif (($str_i == '.') && ((isset($str_im1) && is_numeric($str_im1)) || (isset($str_ip1)) && is_numeric($str_ip1))) {
                $main_array[$count] = (isset($main_array[$count])) ?
                        $main_array[$count] . '.' : '.';
                // Function
            } elseif ((in_array($str_i . $str_ip1 . $str_ip2, $this->valid_functions)) || ($str_i . $str_ip1 . $str_ip2 . $str_ip3 == 'sqrt')) {
                ...
                if (array_key_exists($str_i, $this->variables) && is_numeric($this->variables[$str_i])) {
        ...
        if (!isset($main_array[0]) || strstr('+-*/^', (string) $main_array[0]) || substr_count((string) $main_array[0], '.') > 1) {
            $this->correct = 0;
            return 0;
        }
    ...
    private function checkExp($expression, $length, $cursor, $base) {
        ...
        if ($base < 0) {
            ...
            $test_func = substr($test_func, 0, -strlen(preg_replace('/\s+/', '', $base . substr($expression, $length - $cursor))));
            $test_func_ok = true;
    ...
    private function group(&$formula, $opt = 'init') {
        if ($opt == 'init') {
            ...
            $formula = preg_replace_callback('/[\d\.]+( )?[eE]( )?[\+\-]?( )?[\d\.]+ /', [$this, 'match'], $formula);
            // Variables
            if (count($this->variables) > 0) {
                $valid_variables = implode("|", $this->valid_variables);
                $formula = preg_replace_callback('/[\+\-\*\/\^ ](' . $valid_variables . ')[ \+\-\*\/\^]/', [$this, 'match1'], $formula);
            }
        } else {
            // Functions
            $valid_functions = implode("|", $this->valid_functions);
            $formula = preg_replace_callback('/(' . $valid_functions . ')( )?[\+\-]?( )?[\d\.eEINF]+ /', [$this, 'match2'], $formula);
        }
    }
    ...
    public function getResult() {
        ...
        for ($i = 0; $i <= count($this->valid_variables) - 1; $i++) {
            if (strlen($this->valid_variables[$i]) != 1 || !preg_match('/^([a-z])$/', $this->valid_variables[$i]) === 1 || $this->valid_variables[$i] == "e") {
                $this->correct = 0;
                $this->error_type = 4;
            }
        }
        ...
        if (preg_match('/(\)( )?[^\)\+\-\*\/\^ ])|(\(( )*?\))|([^nstgp\(\+\-\*\/\^ ]( )?\()/', $this->formula)) {
            $this->correct = 0;
        }
        $processing_formula = $this->formula;
        // Begin a general parse
        while ((strstr($processing_formula, '(') || strstr($processing_formula, ')')) && $this->correct) {
            $start_cursor_pos = 0;
            $end_cursor_pos = 0;
...
//        }
        ...
        if ($this->correct) {
            return $this->round_($result, $this->precision);
        } else {
            return false;
        }
    }
