...
class SalesitemsController extends Controller
{
    ...
    public function viewlist($group, $page) {
        ...
        $results = TransactionsDetails::where('transaction_id', $transaction)
                ->get();
        foreach($results as $key => $regtype) {
            if ($regtype->type_product_id != 176) {
                $prod = CatalogProducts::where('id', $regtype->catalog_product_id)
                        ->with('ValorImpuesto')
                        ->first();
            } else {
                $prod = Combination::where('id', $regtype->catalog_product_id)
                        ->with('ValorImpuesto')
                        ->first();
            }
            $results[$key]->producto = $prod;
        }
        // dd($results[2]->producto);
        return Datatables::of($results)
                        ->addColumn('estado', function ($model) {
                            return $model->status == 1 ? __('Active') : __('Inactive');
                        })
                        ->editColumn('tax_iva', function ($data) {
                            // return $data->catalog_products->ValorImpuesto->name;
                            return $data->type_product_id != 176 ? $data->producto->ValorImpuesto->name : "";
                        })
                        ->editColumn('unit_price', function ($data) {
                            return '$ '.number_format($data->unit_price, 2);
                        })
                        ->editColumn('partial_price', function ($data) {
                            return '$ '.number_format($data->quantity*$data->unit_price-$data->iva_value, 2);
                        })
