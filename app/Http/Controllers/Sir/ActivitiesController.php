...
//select(DB::raw("cubes.name as cube, querys2.name parent, querys.* "))
        ...
        $results = Activities::select(DB::raw('activities.id, activities.name, activities.description, activities.status, activities.sort, process.name process'))
                ->join('process', 'process.id', '=', 'activities.process_id');
    ...
    public function edit($group, $page, $id) {
        $activitiesEdit = Activities::where("id", "=", $id)->first();
    ...
    public function save(Request $request, $group, $page) {
        ...
        try {
            $order = Activities::select('sort')->max('sort');
            if(!empty($request->activitiesId)){
                $query = Activities::where("id", "=", $request->activitiesId)->first();
            }
            else {
