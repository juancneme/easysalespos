<?php $page_title = __('Adjust accounting cash') ?>
...
<form action="/{{ $group . '/' . $page }}" method="POST" autocomplete="off" class="form-horizontal"
    ...
    <input type="hidden" name="turno" id="turno" value="{{ !empty($shift) ? $shift->id  : '' }}">
    <input type="hidden" name="user_turno" id="turno" value="{{ !empty($shift) ? $shift->user_id : '' }}">
    ...
    <div class="panel panel-default" style="margin-top: 110px;">
        ...
        @if(!empty($shift))
            @if(Input::get('id') || auth()->user()->hasRole('cajero') )
                <div class="panel-heading">{{ __('Balance sheet') }}  TURNO {{ $shift->id }}
                </div>
            ...
            <div class="input-group control-group increment-item">
                <div class="col-md-11">
                    ...
                    <button type="submit"
                            ...
                            disabled
                            style="display: none"
                            title="{{ __('Save to adjust') }}">{{ __('Save to adjust') }}
                    </button>
                    @else
                    <button type="submit"
                            id="btn-consolidar"
                    ...
                    @endif
                    <button
                        <?= auth()->user()->hasRole('adtienda') || auth()->user()->hasRole('admin') ? '' : 'disabled' ?> 
                        type="button"
                        id="btn-ajust"
...
<script>
    ...
    $itemsdinero = 0;
    var count = 0;
    //se ajusto para que bloquee al admon en turno abierto
    @if(!empty($shift))
        console.log("Información del turno: ", "{{ $shift }}");
        @if($shift->typeshift_id != 14)
            if (window.location.href.includes('closeTurn')) {
                alert('El turno no se puede bloquear');

                @if($shift->typeshift_id == 15 )
                    window.location.href = "{{ url('management/pdvi')}}";
                @elseif($shift->typeshift_id == 12)
                    window.location.href = "{{ url('management/balancesheet')}}";
                @endif
            }
        @else
            @if($hasPendingTransaction > 0 )
                confirm('Favor tramitar las ventas guardadas y domicilios pendientes antes de poder hacer el cierre');
    ...
    function carga_data_items($mIndice, $mItem) {
        ...
        if ($mItem['id'] == '0000000' || $mItem['id'] == '0100092') {
            console.log("escoden ",$mIndice, "#input_amount" + $mIndice,  "#system_amount" + $mIndice, "#diff_amount" + $mIndice);
        ...
        if ($mItem['id'] >= '0300000') {
            $("#input_amount" + $mIndice).attr('readonly', false);
            $("#input_value" + $mIndice).attr('readonly', false);
            $("#input_amount" + $mIndice).attr('style', 'text-align: end; background-color: #bcbfc1;');
            $("#input_value" + $mIndice).attr('style', 'text-align: end; background-color: #bcbfc1;');
        }
    ...
    $('#btn-send').on('click', function () {
        ...
        $(".camsys").attr('hidden', false);
        $(".camdif").attr('hidden', false);
        $(".valmed").attr('readonly', true);
        $(".canmed").attr('readonly', true);

        $("#input_amount" + 1).attr('hidden', true);
        $("#system_amount" + 1).attr('hidden', true);
        $("#diff_amount" + 1).attr('hidden', true);

        $("#input_amount" + 2).attr('hidden', true);
        $("#system_amount" + 2).attr('hidden', true);
        $("#diff_amount" + 2).attr('hidden', true);
    ...
    function ajusteModal(){
        ...
        $('#lbmensaje').text('Esta Seguro de Bloquear el Turno para Ajustes ?');
        $("#ajusteModal").modal("show");
        $("#footer").html('<button type="submit" class="btn btn-primary" data-placement="top"" style="width:100%" >{{__('Aceptar')}} </button> <br><br> <div style="display: flex;justify-content: flex-end;"> <button type="button" class="btn btn-danger" data-dismiss="modal">{{__('Cancel')}}</button> </div>')
    }
