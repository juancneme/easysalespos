...
<div class="panel-body form-add" style="margin-top: 110px;">
    ...
    <form action="/{{ $group . '/' . $page }}" method="POST" enctype="multipart/form-data" class="form-horizontal" style="padding-right: 15px;padding-left: 15px">
        <div style="padding-top:0px; margin-top:5px" class="panel-heading">{{ __('Activities') }}</div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select name="process" id="process-id" class="form-control" required>
                    ...
                    <option value="{{ $p->id }}" {{ !empty($activitiesEdit) && $activitiesEdit->process_id == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <input type="text" name="name" id="name-id" class="form-control" required
                       value="{{ !empty($activitiesEdit) ? $activitiesEdit->name : old('name') }}" />
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <input type="text" name="description" id="description-id" class="form-control"
                       value="{{ !empty($activitiesEdit) ? $activitiesEdit->description : old('description') }}" />
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select name="status" id="estado-name" class="form-control" required>
                    <option <?= !empty($activitiesEdit) && $activitiesEdit->status == "1" ? 'selected' : '' ?> value="1">{{ __('Active') }}</option>
                    <option <?= !empty($activitiesEdit) && $activitiesEdit->status == "0" ? 'selected' : '' ?> value="0">{{ __('Inactive') }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            <div class="col-sm-offset-3 col-sm-6">
                @if(!empty($activitiesEdit))
                <input type="hidden" name="activitiesId" value="{{ $activitiesEdit->id }}" />
                @endif
...
<div class="panel panel-default" style="margin-top: 110px;">
    <div style="padding-top:0px; margin-top:5px" class="panel-heading">{{ __('Activities') }}
...
jQuery(document).ready(function ($) {
    ...
    var table = $(".task-table").DataTable({
        ...
        columns: [ 
            ...
            {data: 'process', name: 'process', title: "{{ __('Process') }}"},
    ...
    table.on( 'row-reorder', function ( e, diff, edit ) {
        ...
        if(result.length > 0){
            ...
            $.ajax({
                url: '/sir/activities/ajax?type=reorder',
                type: 'post',
                data: {
                    "_token": '{{ csrf_token() }}',
                    "result": result
    ...
    @if (!empty($activitiesEdit) || count($errors) > 0)
